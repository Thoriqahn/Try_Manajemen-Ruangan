...
const createBuilding = async (req, res, next) => {
  try {
    const { name, address, lat, lng, image_url, total_floors } = req.body;
    ...
    const id = uuidv4();
    await dbRun('INSERT INTO buildings (id, name, address, lat, lng, image_url, total_floors) VALUES (?,?,?,?,?,?,?)', [id, name, address || null, lat, lng, image_url || null, total_floors || 1]);
...
const updateBuilding = async (req, res, next) => {
  try {
    ...
    await dbRun('UPDATE buildings SET name=?, address=?, lat=?, lng=?, image_url=?, total_floors=? WHERE id=?', [name, address || null, lat, lng, image_url || null, total_floors || 1, req.params.id]);
