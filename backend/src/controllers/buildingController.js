...
const listBuildings = async (req, res, next) => {
  try {
    ...
    for (const b of buildings) {
      b.floors = await dbAll('SELECT * FROM floors WHERE building_id=$1 AND deleted_at IS NULL ORDER BY level', [b.id]);
    }
...
const createBuilding = async (req, res, next) => {
  try {
    ...
    await dbRun('INSERT INTO buildings (id, name, address, lat, lng, image_url, total_floors) VALUES ($1,$2,$3,$4,$5,$6,$7)', [id, name, address || null, lat, lng, image_url, total_floors || 1]);
    await audit({ actorId: req.user.id, actorName: req.user.name, action: 'CREATE_BUILDING', resource: name, ip: req.ip, after: { id, name } });
    const building = await dbGet('SELECT * FROM buildings WHERE id=$1', [id]);
    building.floors = [];
...
const updateBuilding = async (req, res, next) => {
  try {
    ...
    const building = await dbGet('SELECT id, name, image_url FROM buildings WHERE id=$1 AND deleted_at IS NULL', [req.params.id]);
    ...
    await dbRun('UPDATE buildings SET name=$1, address=$2, lat=$3, lng=$4, image_url=$5, total_floors=$6 WHERE id=$7', [name, address || null, lat, lng, image_url, total_floors || 1, req.params.id]);
    ...
    const updated = await dbGet('SELECT * FROM buildings WHERE id=$1', [req.params.id]);
...
const deleteBuilding = async (req, res, next) => {
  try {
    ...
    if (!building) return res.status(404).json({ success: false, message: 'Gedung tidak ditemukan' });
    await dbRun('UPDATE buildings SET deleted_at=NOW() WHERE id=$1', [req.params.id]);
  ...
  try {
    ...
    for (let i = 1; i <= building.total_floors; i++) {
      if (!floors.find(f => f.level === i)) {
        const id = uuidv4();
        await dbRun('INSERT INTO floors (id, building_id, name, level) VALUES ($1,$2,$3,$4)', [id, req.params.id, `Lantai ${i}`, i]);
        hasChanges = true;
      }
    }
    if (hasChanges) {
...
const createFloor = async (req, res, next) => {
  try {
    ...
    const building = await dbGet('SELECT id FROM buildings WHERE id=$1 AND deleted_at IS NULL', [req.params.id]);
    if (!building) return res.status(404).json({ success: false, message: 'Gedung tidak ditemukan' });
    const id = uuidv4();
    await dbRun('INSERT INTO floors (id, building_id, name, level) VALUES ($1,$2,$3,$4)', [id, req.params.id, name, level || 0]);
    const floor = await dbGet('SELECT * FROM floors WHERE id=$1', [id]);
