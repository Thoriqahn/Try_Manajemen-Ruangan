...
const getPolicy = async (req, res, next) => {
  try {
    let policy = await dbGet('SELECT * FROM global_policy WHERE id=1');
    if (!policy) {
      await dbRun('INSERT INTO global_policy (id, max_duration_hours, max_days_ahead) VALUES (1, 4, 30)');
      policy = { id: 1, max_duration_hours: 4, max_days_ahead: 30 };
    }
    const blackoutDates = await dbAll('SELECT * FROM blackout_dates ORDER BY date');
...
const updatePolicy = async (req, res, next) => {
  try {
    ...
    const before = await dbGet('SELECT * FROM global_policy WHERE id=1');
    await dbRun(
      `UPDATE global_policy SET max_duration_hours=COALESCE(?,max_duration_hours), max_days_ahead=COALESCE(?,max_days_ahead), updated_at=strftime('%s','now') WHERE id=1`,
...
const addBlackout = async (req, res, next) => {
  try {
    ...
    await dbRun('INSERT OR IGNORE INTO blackout_dates (id, date, reason) VALUES (?,?,?)', [uuidv4(), date, reason || null]);
...
const removeBlackout = async (req, res, next) => {
  try {
    await dbRun('DELETE FROM blackout_dates WHERE date=?', [req.params.date]);
