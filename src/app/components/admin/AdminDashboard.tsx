...
import { TrendingUp, Users, Clock, AlertTriangle, BarChart2, Calendar, RefreshCw, Monitor, CheckCircle, HelpCircle } from "lucide-react";
...
export function AdminDashboard({ onNavigate, isSuperAdmin = false }: AdminDashboardProps) {
  ...
  const selectedAdmin = adminList.find(a => a.id === selectedAdminFilter);
  const rawRole = selectedAdmin ? selectedAdmin.rawRole : (stats?.rawRole || (isSuperAdmin ? 'SUPERADMIN' : 'ADMIN_RAPAT'));
  const showMeetingRoomStats = ['ADMIN_RAPAT', 'ADMIN', 'SUPERADMIN'].includes(rawRole);
  const showWorkspaceStats = ['ADMIN_KERJA', 'ADMIN', 'SUPERADMIN'].includes(rawRole);

  const meetingStatCards = stats ? [
    { icon: <Calendar size={24} className="text-indigo-500 dark:text-indigo-400" />, label: "Booking Minggu Ini", value: stats.weeklyBookings, bg: "bg-indigo-50 dark:bg-indigo-500/30", border: "border-indigo-100 dark:border-indigo-500/20" },
    { icon: <AlertTriangle size={24} className="text-amber-500 dark:text-amber-400" />, label: "Menunggu Persetujuan", value: stats.pendingApprovals, bg: "bg-amber-50 dark:bg-amber-500/30", border: "border-amber-100 dark:border-amber-500/20" },
    { icon: <Users size={24} className="text-rose-500 dark:text-rose-400" />, label: "Ghost Booking", value: stats.ghostBookings, bg: "bg-rose-50 dark:bg-rose-500/30", border: "border-rose-100 dark:border-rose-500/20" },
    { icon: <Clock size={24} className="text-emerald-500 dark:text-emerald-400" />, label: "Total Ruang Rapat", value: stats.totalRooms, bg: "bg-emerald-50 dark:bg-emerald-500/30", border: "border-emerald-100 dark:border-emerald-500/20" },
  ] : [];

  const workspaceStatCards = stats ? [
    { icon: <Monitor size={24} className="text-blue-500 dark:text-blue-400" />, label: "Total Ruang Kerja", value: stats.totalWorkspaces, bg: "bg-blue-50 dark:bg-blue-500/30", border: "border-blue-100 dark:border-blue-500/20" },
    { icon: <CheckCircle size={24} className="text-emerald-500 dark:text-emerald-400" />, label: "Meja Terisi", value: stats.occupiedDesks, bg: "bg-emerald-50 dark:bg-emerald-500/30", border: "border-emerald-100 dark:border-emerald-500/20" },
    { icon: <HelpCircle size={24} className="text-slate-500 dark:text-slate-400" />, label: "Meja Kosong", value: stats.vacantDesks, bg: "bg-slate-50 dark:bg-slate-500/30", border: "border-slate-200 dark:border-slate-500/20" },
    { icon: <AlertTriangle size={24} className="text-amber-500 dark:text-amber-400" />, label: "Pending Requests", value: stats.pendingWorkspaceRequests, bg: "bg-amber-50 dark:bg-amber-500/30", border: "border-amber-100 dark:border-amber-500/20" },
  ] : [];
          ...
          {showWorkspaceStats && (
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">Statistik Ruang Kerja</h3>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
                {workspaceStatCards.map((s, i) => (
              ...
              <div className="bg-white/90 backdrop-blur-md border border-slate-200 rounded-2xl p-6 shadow-sm transition-colors dark:bg-slate-800/90 dark:border-slate-700/50">
                <h3 className="text-slate-800 font-bold mb-4 transition-colors dark:text-slate-100">Tingkat Okupansi Total</h3>
                <div className="w-full bg-slate-100 h-4 rounded-full overflow-hidden shadow-inner transition-colors dark:bg-slate-700/50">
                  <div 
                    className="bg-blue-500 h-full rounded-full transition-all duration-1000 ease-out"
                    style={{ width: `${stats?.totalDesks > 0 ? (stats.occupiedDesks / stats.totalDesks) * 100 : 0}%` }}
                  />
                </div>
                <div className="flex justify-between mt-2 text-xs font-medium text-slate-500 dark:text-slate-400">
                  <span>{stats?.occupiedDesks || 0} Terisi</span>
                  <span>{stats?.totalDesks || 0} Kapasitas Maksimal</span>
                </div>
              </div>
            </div>
          )}

          {showMeetingRoomStats && (
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500 delay-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">Statistik Ruang Rapat</h3>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
                {meetingStatCards.map((s, i) => (
                  <div key={i} className="bg-white/90 backdrop-blur-md border border-slate-200 rounded-2xl p-6 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group dark:bg-slate-800/90 dark:border-slate-700/50">
                    <div className={`w-12 h-12 ${s.bg} border ${s.border} rounded-xl flex items-center justify-center mb-4 transition-transform group-hover:scale-110 duration-300`}>{s.icon}</div>
                    <div className="text-slate-500 text-xs font-bold uppercase tracking-wider mb-2 transition-colors dark:text-slate-400">{s.label}</div>
                    <div className="text-3xl font-extrabold text-slate-800 tracking-tight transition-colors dark:text-slate-100">{s.value ?? "—"}</div>
      ...
      <div className="bg-white/90 backdrop-blur-md border border-slate-200 rounded-2xl p-6 shadow-sm transition-colors dark:bg-slate-800/90 dark:border-slate-700/50">
        ...
        <div className="flex flex-wrap gap-4">
          <button onClick={() => onNavigate("admin-approval")} className="px-5 py-3 bg-amber-50 border border-amber-200 text-amber-700 text-sm font-bold rounded-xl hover:bg-amber-100 dark:hover:bg-amber-500/20 transition-all flex items-center gap-2.5 active:scale-95 shadow-sm dark:bg-amber-500/30 dark:text-amber-400 dark:border-amber-500/20">
            ...
            Lihat Persetujuan Pending {((stats?.pendingApprovals || 0) + (stats?.pendingWorkspaceRequests || 0)) > 0 && <span className="bg-amber-500 text-white text-[10px] px-2 py-0.5 rounded-md shadow-sm transition-colors duration-300 dark:bg-amber-600 dark:text-slate-900">{(stats?.pendingApprovals || 0) + (stats?.pendingWorkspaceRequests || 0)}</span>}
          </button>
