...
export function GlobalPolicy() {
  ...
  const TooltipWrapper = ({ text, children }: { text: string; children: React.ReactNode }) => (
    <div className="relative group inline-flex items-center gap-1">
      {children}
      <div className="absolute bottom-full left-0 mb-2 w-56 bg-gray-800 text-white text-xs rounded-lg px-3 py-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 shadow-lg">{text}</div>
    </div>
  );
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500">Aturan yang berlaku untuk seluruh sistem</p>
      </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          ...
          <div className="space-y-5">
            ...
            <div className="pt-3 border-t border-gray-100">
              <p className="text-xs text-gray-400">Kebijakan ini berlaku untuk seluruh pengguna biasa. Admin & Super Admin tidak terikat batasan ini.</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1))} className="p-1 hover:bg-gray-100 rounded-lg"><ChevronLeft size={16} className="text-gray-500" /></button>
              <span className="text-sm text-gray-700" style={{ fontWeight: 600 }}>{monthNames[month]} {year}</span>
              <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1))} className="p-1 hover:bg-gray-100 rounded-lg"><ChevronRight size={16} className="text-gray-500" /></button>
            </div>
            <div className="grid grid-cols-7 gap-1 mb-1">
              {["Sen","Sel","Rab","Kam","Jum","Sab","Min"].map(d => <div key={d} className="text-center text-xs text-gray-400 py-1" style={{ fontWeight: 500 }}>{d}</div>)}
            </div>
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <button key={day} onClick={() => !isWeekend && toggleBlackout(dateStr)} onMouseEnter={() => {}} disabled={isWeekend}
                    className={`aspect-square flex items-center justify-center text-xs rounded-lg transition-all ${isBlackout ? "bg-red-500 text-white" : isWeekend ? "text-gray-300 cursor-default" : "hover:bg-red-50 hover:text-red-600 text-gray-600 cursor-pointer"}`}>
                    {day}
                  </button>
                );
              })}
