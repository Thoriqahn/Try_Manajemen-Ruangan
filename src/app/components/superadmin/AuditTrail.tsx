                          ...
                          <div className="space-y-2">
                            ...
                            {(log.payload_before || log.payload_after) && (
                              <div className="grid grid-cols-2 gap-3">
                                {log.payload_before && <div className="bg-red-50 border border-red-100 rounded-xl p-3 transition-colors duration-300 dark:bg-red-900/20 dark:border-red-800/30"><div className="text-xs text-red-500 mb-1" style={{ fontWeight: 600 }}>SEBELUM</div><pre className="text-xs text-gray-600 font-mono overflow-auto transition-colors duration-300 dark:text-slate-300">{typeof log.payload_before === "string" ? log.payload_before : JSON.stringify(log.payload_before, null, 2)}</pre></div>}
                                {log.payload_after && <div className="bg-green-50 border border-green-100 rounded-xl p-3 transition-colors duration-300 dark:bg-green-900/20 dark:border-green-800/30"><div className="text-xs text-green-600 mb-1 transition-colors duration-300 dark:text-green-400" style={{ fontWeight: 600 }}>SESUDAH</div><pre className="text-xs text-gray-600 font-mono overflow-auto transition-colors duration-300 dark:text-slate-300">{typeof log.payload_after === "string" ? log.payload_after : JSON.stringify(log.payload_after, null, 2)}</pre></div>}
                              </div>
                            )}
                          </div>
                        </td>
