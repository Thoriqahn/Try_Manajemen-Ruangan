    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      {loading ? (
        ...
        <div className="bg-white backdrop-blur-md rounded-2xl shadow-md shadow-black/5 dark:shadow-black/20 border border-gray-100 p-12 text-center flex flex-col items-center justify-center min-h-[400px] transition-colors duration-300 dark:bg-slate-800 dark:border-slate-700">
          ...
          <h3 className="text-xl font-bold text-gray-800 mb-2 transition-colors duration-300 dark:text-slate-100">Belum Ada Gedung</h3>
          <p className="text-gray-500 mb-6 max-w-md transition-colors duration-300 dark:text-slate-400">Sistem belum memiliki data gedung. Silakan tambahkan gedung pertama Anda untuk memulai manajemen ruangan.</p>
          <button onClick={() => { setEditBuilding(null); setShowForm(true); }} className="flex items-center gap-2 bg-[#1E3A5F] text-white px-6 py-3 rounded-2xl text-sm hover:bg-[#0F2144] transition-all font-semibold shadow-md">
          ...
          {buildings.map(b => (
            <div key={b.id} className="group bg-white backdrop-blur-md rounded-2xl shadow-md shadow-black/5 dark:shadow-black/20 border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 flex flex-col relative dark:bg-slate-800 dark:border-slate-700">
              <div className="absolute top-3 right-3 z-10 flex gap-2 transition-opacity transition-all duration-300">
                ...
                <button onClick={() => handleDelete(b.id, b.name)} className="p-2 bg-white/90 backdrop-blur text-red-600 hover:bg-white backdrop-blur-md rounded-xl shadow-md shadow-black/5 dark:shadow-black/20 transition-all duration-300 dark:bg-slate-900/90 dark:text-red-400" title="Hapus">
                  <Trash2 size={16} />
                </button>
              </div>
              
              <div className="h-48 bg-gray-100 relative overflow-hidden transition-colors duration-300 dark:bg-slate-800">
                {b.image_url ? (
                  <img src={getImageUrl(b.image_url)} alt={b.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500 transition-all duration-300" />
                ) : (
              ...
              <div className="p-5 flex flex-col gap-4 flex-1">
                <div className="flex items-start gap-2 text-gray-600 text-sm transition-colors duration-300 dark:text-slate-300">
                  <MapPin size={16} className="text-blue-500 shrink-0 mt-0.5" />
                  <p className="line-clamp-2 leading-relaxed">{b.address || "Alamat belum disetel"}</p>
                </div>
                
                <div className="flex items-center gap-4 mt-auto pt-4 border-t border-gray-100 transition-colors duration-300 dark:border-slate-800/50">
                  <div className="flex items-center gap-1.5 text-sm font-medium text-gray-700 bg-gray-50 px-3 py-1.5 rounded-xl transition-colors duration-300 dark:bg-slate-800/80 dark:text-slate-200">
                    <Layers size={15} className="text-purple-500" />
                    <span>{b.total_floors || 1} Lantai</span>
                  </div>
                  {b.lat && b.lng && (
                    <button onClick={() => window.open(`https://www.google.com/maps?q=${b.lat},${b.lng}`, '_blank')} className="text-xs font-semibold text-blue-600 hover:text-blue-800 ml-auto flex items-center gap-1 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-xl transition-colors dark:bg-blue-900/40 dark:text-blue-200">
                      Lihat Peta
                    </button>
                  )}
                </div>
...
function BuildingFormModal({ building, onClose }: { building: any; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 sm:p-6">
      <div className="bg-white backdrop-blur-md rounded-2xl w-full max-w-5xl shadow-2xl flex flex-col max-h-[95vh] overflow-hidden transition-colors duration-300 dark:bg-slate-900/90">
        <div className="bg-gray-50 px-6 py-4 border-b border-gray-100 flex items-center justify-between shrink-0 transition-colors duration-300 dark:bg-slate-800/80 dark:border-slate-800/50">
          <div>
            <h3 className="text-lg font-bold text-gray-800 transition-colors duration-300 dark:text-slate-100">{building ? "Edit Profil Gedung" : "Daftarkan Gedung Baru"}</h3>
            <p className="text-xs text-gray-500 mt-1 transition-colors duration-300 dark:text-slate-400">Lengkapi data properti dan koordinat lokasinya.</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col md:flex-row flex-1 overflow-y-auto">
          ...
          <div className="w-full md:w-5/12 bg-gray-50 p-6 border-r border-gray-100 space-y-6 flex flex-col shrink-0 transition-colors duration-300 dark:bg-slate-800/80 dark:border-slate-800/50">
            ...
            {/* Foto Gedung */}
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2 transition-colors duration-300 dark:text-slate-200">Foto Gedung <span className="text-red-500">*</span></label>
              <div 
                onClick={() => fileInputRef.current?.click()}
                className={`relative w-full h-48 rounded-2xl border-2 border-dashed overflow-hidden flex flex-col items-center justify-center cursor-pointer transition-all ${previewUrl ? 'border-transparent shadow-md' : 'border-gray-300 dark:border-slate-600 hover:border-blue-500 hover:bg-blue-50/50'}`}
              >
                {previewUrl ? (
                  <>
                    <img src={previewUrl} alt="Preview" className="w-full h-full object-cover" />
                    <div className="absolute inset-0 bg-black/40 hover:opacity-100 transition-opacity flex items-center justify-center transition-all duration-300">
                      <span className="text-white text-sm font-medium flex items-center gap-2"><Edit2 size={16}/> Ganti Foto</span>
                    </div>
                  </>
                ) : (
                  <div className="text-center p-4">
                    <div className="w-12 h-12 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mx-auto mb-3 transition-colors duration-300 dark:bg-blue-900/40 dark:text-blue-400">
                      <UploadCloud size={24} />
                    </div>
                    <p className="text-sm font-medium text-gray-700 transition-colors duration-300 dark:text-slate-200">Pilih atau Tarik Gambar</p>
                    <p className="text-xs text-gray-500 mt-1 transition-colors duration-300 dark:text-slate-400">Maks. 1 MB (JPG/PNG/WEBP)</p>
                  </div>
                )}
            ...
            <div className="flex-1 flex flex-col min-h-[250px]">
              <div className="flex justify-between items-center mb-2">
                <label className="block text-sm font-semibold text-gray-700 transition-colors duration-300 dark:text-slate-200">Pinpoint Peta <span className="text-red-500">*</span></label>
                {(form.lat && form.lng) && (
                  <span className="text-[10px] font-medium bg-green-100 text-green-700 px-2 py-0.5 rounded-full transition-colors duration-300 dark:bg-green-900/40 dark:text-green-300">Koordinat Disetel</span>
                )}
              </div>
              <p className="text-xs text-gray-500 mb-3 leading-relaxed transition-colors duration-300 dark:text-slate-400">Klik pada peta di bawah ini untuk menentukan titik koordinat gedung.</p>
              
              <div className="flex-1 rounded-2xl overflow-hidden border border-gray-200 shadow-inner z-0 relative min-h-[200px] transition-colors duration-300 dark:border-slate-700/50">
          ...
          <div className="w-full md:w-7/12 p-6 bg-white backdrop-blur-md space-y-6 flex flex-col justify-between transition-colors duration-300 dark:bg-slate-900/90">
            <div className="space-y-5">
              ...
              <div className="pt-4 mt-4 border-t border-gray-100 grid grid-cols-2 gap-4 transition-colors duration-300 dark:border-slate-800/50">
                <div>
                  <label className="block text-xs font-semibold text-gray-500 mb-1 transition-colors duration-300 dark:text-slate-400">Latitude</label>
                  <div className="p-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-600 font-mono select-all transition-colors duration-300 dark:bg-slate-800/80 dark:text-slate-300 dark:border-slate-700/50">
                    {form.lat || "Belum disetel"}
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-500 mb-1 transition-colors duration-300 dark:text-slate-400">Longitude</label>
                  <div className="p-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-600 font-mono select-all transition-colors duration-300 dark:bg-slate-800/80 dark:text-slate-300 dark:border-slate-700/50">
            ...
            <div className="flex justify-end gap-3 pt-6 border-t border-gray-100 transition-colors duration-300 dark:border-slate-800/50">
              <button type="button" onClick={onClose} className="px-6 py-2.5 border border-gray-200 text-gray-600 rounded-2xl text-sm font-medium hover:bg-gray-50 transition-colors dark:bg-slate-800/80 dark:text-slate-300 dark:border-slate-700/50">Batal</button>
              <button type="submit" disabled={loading} className="px-6 py-2.5 bg-[#1E3A5F] text-white rounded-2xl text-sm font-semibold hover:bg-[#0F2144] transition-colors disabled:opacity-70 flex items-center gap-2 shadow-md">
                {loading ? (
